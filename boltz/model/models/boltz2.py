...
class Boltz2(LightningModule):
    ...
    def forward(
        ...
        cached_structure: Optional[dict[str, Tensor]] = None,
    ) -> dict[str, Tensor]:
        ...
        if self.confidence_prediction and not cached:
        ...
        if self.affinity_prediction:
            ...
            best_idx = 0 if cached else torch.argsort(dict_out["iptm"], descending=True)[0].item()
    ...
    def training_step(self, batch: dict[str, Tensor], batch_idx: int) -> Tensor:
        ...
        if not (self.global_step % self.log_loss_every_steps):
            ...
            if self.confidence_prediction and "plddt" in out:
    ...
    def predict_step(self, batch: Any, batch_idx: int, dataloader_idx: int = 0) -> dict:
        try:
            out = self(
                ...
                cached_structure=(
                    {"s": batch["cached_s"], "z": batch["cached_z"], "coords": batch["cached_coords"]}
                    if "cached_s" in batch
                    else None
                ),
            )
