...
class PredictionDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        if self.affinity:
            ...
            # Reuse the structure-pass trunk state after applying the exact
            # affinity crop. Cache files are written by BoltzWriter and use
            # the original token/atom numbering.
            cache_path = (
                self.target_dir
                / record.id
        ...
        if self.affinity and cached_s is not None:
            atom_count = features["atom_pad_mask"].shape[0]
            if cached_coords.shape[0] > atom_count:
                raise ValueError("Cached coordinates exceed affinity atom axis")
